...
- `dataset/` — the POC dataset itself (222 works, 706 releases, 8 platforms, CC0 from Wikidata). Read its `README.md` before touching it: the CanonicalIds are **final** (invariant 9), so regenerating mints a *new* referential rather than updating this one.
