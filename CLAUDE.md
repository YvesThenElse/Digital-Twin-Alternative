...
| `./controles.sh` | the four **offline** dataset checks — no network, no .NET, no Node, only python3 |
...
⚠️ **An orchestration step that fails must name what failed.** `services_front` did `./web.sh build >/dev/null`: a typing error went into the void, and `set -e` made `e2e.sh` exit 1 **without a word**, right after printing "── front" — so one looks for the defect in the product while it sits in the tooling. Heavy steps now go through `services_etape`, silent while they succeed and verbose when they fail, and `web/src/outillage/orchestration.test.ts` refuses a build or a migration whose output is discarded.
...
`calibration/` holds Python scripts that build and measure the dataset against Wikidata and Wikipédia. They are instruments, not architecture — they imply nothing about the stack. The four offline checks now run **from anywhere**, via `./controles.sh` or individually: each resolves its paths from its own file, not from the working directory. Audit item 37 found the opposite, and one claim here was simply false — `test_covers.py` does *not* read versioned files only: the 218 cover images are deliberately absent from this public repo (`.gitignore`, conditions 4 and 5 of `VERIFICATION-JURIDIQUE.md` §3.3). On a fresh checkout it therefore exits **2** — « I could not run » — and names `fetch_covers.py`, which is distinct from **1**, « I found a fault ». A check that returned 0 without reading a single file would be worse than no check: `controles.sh` would announce that all four passed.

Four of them are offline checks to run after touching the pipeline — `./controles.sh` runs all four and distinguishes a failure from a check that could not run: `test_id_stability.py` (invariant 9 under three perturbations), `test_wp_parser.py` (thirteen real infobox cases, on cached wikitext), `emit_notabilite.py` (regenerates `dataset/NOTABILITE.md`, which must never be hand-edited), and `test_covers.py` (the five written cover conditions of `VERIFICATION-JURIDIQUE.md` §3.3 — manifest/file agreement, provenance present, byte count, and the 512 px cap). The manifest recorded `width: 512` for all 218 covers while the files measured 213–960 px: it was the size *requested*, not obtained, and `bytes` was exact — within one record some fields were observations and others intentions.
...
- `BOUCLE-FINITION.md` + `TODO-FINITION.md` — the loop that follows the audit: it **builds**, but only what is already written. Fifteen items ordered by what blocks a tester first, and a section that explicitly **stops the loop and asks** — four items still need a product decision. It carries the audit's lessons as working rules: a `data-*` attribute is not a rendering, a rule written in the client does not exist, covering cases is not covering a set, measure artefacts instead of reading what they say about themselves.
- `BOUCLE-ECRANS.md` + `TODO-ECRANS.md` — the loop that **finishes Phase 1**. Three lines of its written perimeter were never delivered (search, "a few statistics", "simple profile page") and four screen fiches declare themselves Phase 1 without existing: E04's merged header, E05, E06, E07. Nine items plus two decisions — one of which, the statistics, *contradicts a deferral written an hour earlier*, and the other is whether E07 comes before or after the user test.
...
- `web/src/styles/socle.css` — the visual foundation: warm neutrals, the six era accents, typography, the base-4 spacing scale, breakpoints, motion. It did not exist until 22 Sept 2026, and nothing flagged that: 198 front tests and an end-to-end journey were green because they asserted `data-*` attributes — declared intentions. Two guards now hold it. `disposition/palette.test.ts` mirrors the six accents against `EPOQUES` and refuses a seventh; the journey checks the served page has the warm `rgb(250, 248, 245)` ground and 44 px targets, and fails **naming the stylesheet** if the import goes. A component test cannot see any of this — it renders into a document without CSS.
