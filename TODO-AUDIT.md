...
- [x] **22 — Décocher ne se persiste pas.** (E02 : « chaque bascule est persistée immédiatement ») Le geste le plus fréquent de l'écran ne quitte pas le navigateur, et depuis la relecture d'état il se défait sous les yeux de l'utilisateur au premier rechargement. *Décision attendue : que signifie une rétractation dans un journal en ajout seul ? Remplacer l'événement par une correction (`MarkSupersededAsync` existe, aucun point d'entrée ne l'expose), ou enregistrer une déclaration négative distincte ? Acceptation : décocher survit à un rechargement, et le parcours de bout en bout le vérifie.*
  ...
  **Tranché et fait le 22 septembre 2026 : l'événement est REMPLACÉ**, ce que le modèle prévoyait déjà. §5.3 fait de la correction « une fonctionnalité de premier plan » dont la trace est « conservée côté système sans être exposée » — le journal garde tout, l'axe et la sélection ne montrent plus rien. `POST /declarations/retract` marque les événements vivants de l'œuvre et retire son jugement permanent, sinon « je l'avais » survivrait à un jeu qu'on ne déclare plus. Le geste est **rejouable** : un double tap ne casse rien. Le parcours coche par erreur, corrige, **recharge**, et vérifie que la correction a tenu — mutation confirmée sur les deux dispositions.
...
- [x] **26 — Le moment ne dit pas ce qu'il est.** (E03, langage visuel §3) `type` et `targetKind` sont rendus par l'API et jetés par l'écran : trois moments d'un même jeu s'affichent en trois lignes identiques, et un titre saisi se donne pour une œuvre curée. *Le langage visuel prescrit une **icône dessinée** par famille — ce qu'on a fait du jeu, où l'objet se trouvait, ce qu'il a laissé — et « aucune légende ». Décision attendue : dessiner ces icônes, ou regrouper les moments d'un même jeu (E03 repère A parle de regroupement au niveau décennie).*

  **Fait le 22 septembre 2026.** Les icônes de §3 existent — trois familles qui se distinguent par leur **sujet** et non par leur géométrie, la fiche ayant écarté un système de formes « élégant sur le papier et illisible à l'écran ». `type` et `targetKind` sont désormais rendus : le jeu affiné du parcours porte trois marques distinctes et **nommées**, et un titre saisi ne se donne plus pour une œuvre curée. Les sept types sans producteur ne sont pas peints en « joué » : l'axe dit qu'il ne sait pas, ce qui est une information.
