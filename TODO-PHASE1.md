...
- [x] **17 — Servir les jaquettes.** L'API annonce `/covers/{id}` et personne ne les rend : la grille desktop afficherait 218 images cassées, alors que la reconnaissance est la mécanique centrale de E02. *Acceptation : un test vérifie qu'une URL annoncée **résout** — l'item 13 ne vérifiait que le cas `null` —, et le parcours de bout en bout échoue si une image de la grille ne charge pas.*
...
- **17** — `/covers/{id}` sert enfin les jaquettes annoncées, depuis le manifeste et jamais depuis la requête — accepter un nom de fichier laisserait remonter l'arborescence. Le catalogue n'annonce que les jaquettes **présentes sur le disque** : le manifeste enregistre une acquisition, pas une présence, et un clone neuf n'a aucune image. **Le parcours a trouvé une seconde frontière** : le mandataire de Vite ne relaie que `/api`, si bien que le navigateur demandait `/covers/…` au serveur du front et recevait du HTML. Les deux côtés avaient raison séparément ; personne ne possédait la frontière. **Une mutation a montré qu'un de mes tests ne testait rien** : « chaque jaquette annoncée existe » est vrai avec ou sans le filtre, les 218 fichiers étant là — il a fallu fabriquer le clone neuf dans un dossier temporaire. 515 tests .NET, 120 tests front, parcours vert sur les deux dispositions avec vérification du **chargement** de chaque image.
